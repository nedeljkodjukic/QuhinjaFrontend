        ...
        <q-form class="full-width" ref="formRef" @submit="handleAddRecipe">
          <div class="full-width row justify-around wrap q-gutter-x-md">
            <div class="full-width column justify-center items-center" style="max-width: 350px">
              ...
              <q-btn style="background-color: #6f6e57" label="Potvrdi" text-color="white" type="submit" />
            </div>
            ...
            <div class="flex column">
              <div class="flex q-mb-md full-width" style="max-width: 350px">
                ...
                <q-list bordered class="bg-grey-5 rounded-borders" style="max-width: 600px">
                  ...
                  <q-item style="width: 345px" v-for="ing in ingridientsForBase" :key="ing.ingridientName.label">
                    <q-item-section top class="gt-sm">
                      <q-item-label class="q-mt-sm">{{ ing.ingridientName.label }} : {{ ing.ingridientAmount }} : {{ ing.ingridientUnit }}</q-item-label>
                    </q-item-section>

                    <q-item-section top side>
                      <div class="text-grey-8 q-gutter-xs">
                        <q-btn @click="IncrementAmount(ing.ingridientName.label)" class="gt-xs" size="12px" flat dense round icon="add" />
                        <q-btn @click="DecrementAmount(ing.ingridientName.label)" class="gt-xs" size="12px" flat dense round icon="remove" />
                        <q-btn @click="deleteItem(ing.ingridientName.label)" size="12px" flat dense round icon="delete" />
                      </div>
                    </q-item-section>
                  </q-item>
                </q-list>
              ...
              <q-uploader color="teal-10" ref="uploaderRefForRecipe" style="max-width: 300px" class="full-width" :factory="factoryUpload" hide-upload-btn multiple batch />
            </div>
          </div>
        </q-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    IncrementAmount(name) {
      var ar = this.ingridientsForBase.filter((el) => el.ingridientName.label == name).map((el) => el.ingridientAmount++);
    },
    DecrementAmount(name) {
      var ar = this.ingridientsForBase.filter((el) => el.ingridientName.label == name).map((el) => (el.ingridientAmount > 1 ? el.ingridientAmount-- : ""));
    },
    deleteItem(name) {
  ...
  created() {
    this.getDish();
    this.$store.dispatch("apiRequest/getApiRequest", { url: `/ingridient` }).then((res) => {
      res.forEach((el) => {
        var Option = {
          label: el.name,
          disable: false,
