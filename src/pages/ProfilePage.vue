<template>
  ...
  <q-form class="full-width text-center column q-gutter-y-sm" style="max-width: 350px" @submit="handleSubmit">
    ...
    <q-select v-model="town" label="Grad" outlined dense :options="townOptions" :rules="[requiredField]"
              use-input
              hide-selected
              fill-input
              input-debounce="0"  @filter="filterTowns"/>
...
export default {
  ...
  methods: {
    getData () {
      ...
      this.$store.dispatch('apiRequest/getApiRequest', { url: 'user/0' })
        .then(res => {
          (this.userData = res)
          (console.log(res))
    ...
    handleSubmit () {
      this.$store
        ...
        .then(res => { this.$refs.uploaderRef.upload() })
